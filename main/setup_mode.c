...
static const char k_index_html[] =
    ...
    ".toolbar{display:flex;gap:10px;flex-wrap:wrap;align-items:stretch;justify-content:flex-start;"
    "margin-bottom:15px;padding:10px;border:1px dashed var(--cyan);}"
    ...
    "text-transform:uppercase;font-family:inherit;font-size:0.8rem;transition:0.2s;width:140px;text-align:center;}"
    ...
    "<button class=\"btn\" onclick=\"fsRename()\">[R] RENAME</button>"
    "<button class=\"btn\" onclick=\"fsMkdir()\">[+] NEW DIR</button>"
    "<button class=\"btn\" style=\"border-color:var(--red);color:var(--red);\" onclick=\"fsDelete()\">[x] DELETE</button>"
    "<button id=\"btnCancel\" class=\"btn\" style=\"border-color:var(--red);color:var(--red);\" onclick=\"cancelTransfer()\" disabled>[!] CANCEL</button>"
    ...
    "tr.innerHTML='<td>'+(i.type==='dir'?'[DIR]':'[FILE]')+'</td><td>'+i.name+'</td><td>'+(i.size||'')+'</td><td>'+dateStr+'</td>';"
    ...
    "pc.style.display='block';setCancelEnabled(true);uploadRaw(file,true);}"
    ...
    "function setCancelEnabled(on){document.getElementById('btnCancel').disabled=!on;}"
    "function resetTransferUi(){setCancelEnabled(false);if(pb){pb.style.width='0%';}if(pt){pt.textContent='';}"
    ...
    "pc.style.display='block';setCancelEnabled(true);const path=(currentPath==='/'?'/':currentPath+'/')+selected.name;"
