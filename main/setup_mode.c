...
static const char k_index_html[] =
    ...
    "</div></div><script>"
    "let currentPath='/';let selected=null;let uploading=false;let downloading=false;let filled=false;let pc=null;let pb=null;let pt=null;"
    ...
    "pc.style.display='block';const path=(currentPath==='/'?'/':currentPath+'/')+selected.name;"
    "const url='/api/fs/download?path='+encodeURIComponent(path);const fname=selected.name;"
    "const done=()=>{downloading=false;pc.style.display='none';};"
    "try{if(window.showSaveFilePicker){"
    "const handle=await window.showSaveFilePicker({suggestedName:fname});"
    "const writable=await handle.createWritable();const res=await fetch(url);"
    ...
    "const xhr=new XMLHttpRequest();const start=performance.now();xhr.responseType='blob';"
    ...
    "xhr.onerror=()=>{alert('Download failed');done();};xhr.open('GET',url);xhr.send();}}"
